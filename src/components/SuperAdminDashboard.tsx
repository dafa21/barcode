      ...
      <div className="col-span-12 lg:col-span-4 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          <button 
            ...
            className="w-8 h-8 flex items-center justify-center bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-lg hover:from-emerald-500 hover:to-teal-500 transition-all duration-300 shadow-sm shadow-emerald-500/25 transform hover:-translate-y-0.5"
          >
        ...
        {showNewOffice && (
          <div className="bg-white p-4 rounded-xl border border-gray-200">
            <h3 className="text-[10px] uppercase tracking-widest text-emerald-700 mb-3">Provision New Tenant</h3>
            <form onSubmit={handleCreateOffice} className="space-y-3">
              ...
              <div className="flex justify-end gap-2 mt-4">
                ...
                <button type="submit" className="px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg">Deploy</button>
              </div>
            </form>
          </div>
        )}
            ...
            <button
              ...
              className={`w-full text-left flex items-center justify-between p-5 rounded-2xl border transition-all duration-300 group ${
                selectedOffice?.id === office.id 
                  ? 'bg-gradient-to-br from-emerald-50 to-teal-50 border-emerald-200 shadow-md ring-1 ring-emerald-500/20' 
                  : 'bg-white border-gray-100 hover:border-emerald-200 hover:shadow-lg hover:-translate-y-0.5'
              }`}
            >
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${selectedOffice?.id === office.id ? 'bg-emerald-600 text-white shadow-md shadow-emerald-500/20' : 'bg-gray-50 text-gray-500 group-hover:bg-emerald-50 group-hover:text-emerald-600'}`}>
                  <Building2 className="w-5 h-5" />
                </div>
                <div>
                  <div className={`text-sm mb-0.5 transition-colors ${selectedOffice?.id === office.id ? 'text-emerald-900 font-bold' : 'font-semibold text-gray-900 group-hover:text-emerald-600'}`}>{office.officeName}</div>
                  {office.contactEmail && <div className="text-[10px] uppercase tracking-widest text-gray-500 font-bold">{office.contactEmail}</div>}
                </div>
              </div>
              <ChevronRight className={`w-5 h-5 transition-transform ${selectedOffice?.id === office.id ? 'text-emerald-600 translate-x-1' : 'text-gray-400 group-hover:text-emerald-500 group-hover:translate-x-1'}`} />
            </button>
          ))}
          ...
          <div className="bg-white/90 backdrop-blur-xl rounded-3xl border border-gray-100 p-6 md:p-8 flex flex-col flex-1 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
            <div className="mb-6 pb-6 border-b border-gray-200 flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] uppercase tracking-widest text-gray-500 mt-1">Tenant ID: <span className="font-mono text-emerald-600">{selectedOffice.id}</span></p>
              </div>
            ...
            <div className="max-w-md">
              ...
              <form onSubmit={handleCreateAdmin} className="space-y-4">
                ...
                <button type="submit" className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition-colors text-[10px] font-bold uppercase tracking-widest shadow-lg mt-2">
