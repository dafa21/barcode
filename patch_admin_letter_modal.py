          ...
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl relative flex flex-col">
            ...
            <div className="p-8 bg-gray-50 flex justify-center items-start min-h-[500px]">
              ...
              >
                ...
                <div className="relative z-10 font-serif text-gray-800 leading-relaxed whitespace-pre-wrap">
                  {(selectedEvent.letterContent || '').replace(/\{\{nama_tamu\}\}/g, printingGuest.guestName)}
                </div>
              </div>
            </div>
...
"""
    ...
    # insert before final </div></div> (which is the end of return)
    ...
    #   );
    # }
    # So I will search for the last </div>
    last_div = "    </div>\n  );\n}"
    if last_div in content:
